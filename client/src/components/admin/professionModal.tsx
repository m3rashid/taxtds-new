...
const ProfessionModal: React.FC<IProps> = ({
  data: gotData,
  showModal,
  setShowModal,
}) => {
  ...
  const completeProcess = async () => {
    ...
    try {
      ...
      if (showModal === "edit") {
        body = JSON.stringify({
          professionId: gotData.id,
          name: data,
        });
      } else {
        body = JSON.stringify({
          professionId: gotData,
        });
      }
      const res = await axios.post(
        SERVER_ROOT_URL + "/admin/professions/" + showModal,
        body,
        tokenHeader
      );
      close();
    ...
    <ModalWrapper close={close}>
      {showModal === "delete" ? (
        <div className="text-lightBgOne mb-6 text-center">
          ...
          <p>
            This will affect all listings and users who have this as a
            profession
          </p>
